...
import MorphingBG from './morph';

import './style.scss';


class MorphBackground extends React.Component { // eslint-disable-line react/prefer-stateless-function
	constructor(props) {
		super(props);
	}

	componentDidMount(){
		document.querySelector('svg.scene') &&
		new MorphingBG(document.querySelector('svg.scene'));
	}

	render() {
		return (
			<div className="morph-background" dangerouslySetInnerHTML={{__html:`
